...
<body> 
    <nav class="navbar navbar-expand-lg py-3">
        <div class="container d-flex justify-content-between align-items-center">
            ...
            <a href="{{ route('orcamento.index') }}" class="btn btn-outline-dark" style="border-radius: 0; font-size: 0.85rem; text-transform: uppercase;">
                Ver Orçamento ({{ count(session('orcamento', [])) }})
            </a>
        </div>
    </nav>
    ...
    <div class="container py-5">
        ...
        <div class="row g-4 mt-2">
            ...
            <div class="col-md-4 mb-5">
                <div class="card card-joia">
                    ...
                    <div class="card-body text-center d-flex flex-column">
                        <span class="product-sku">{{ $produto->sku }}</span>
                        <h5 class="product-title">{{ $produto->name }}</h5>
                        <p class="text-muted small mb-4">{{ $produto->plating_details }}</p>
                        
                        <form action="{{ route('orcamento.add') }}" method="POST" class="mt-auto">
                            @csrf
                            <input type="hidden" name="produto_id" value="{{ $produto->sku }}">
                            <input type="hidden" name="produto_nome" value="{{ $produto->name }}">
            ...
            <a href="{{ route('produtos.catalogo') }}" class="btn-primario">
